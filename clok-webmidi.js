  ...
  if (!global.ClokTC || !global.ClokState || !global.ClokEngine ||
    ...
    class MtcGenerator {
      constructor({ send, fps = 25, onEvent = null } = {}) {
        ...
        // Confirmed position — what update()'s drift check compares
        // against, and what the UI/callers read via the hh/mm/ss/ff
        // getters. Kept in step with whatever's been scheduled as it's
        // scheduled (see _advanceSchedFrame), since that's the position
        // Pro Tools will actually be at once those messages land.
        this._hh = 0; this._mm = 0; this._ss = 0; this._ff = 0;

        // ── Lookahead scheduler state ────────────────────────────────
        // Console logging (see clok-webmidi.js console output from the
        // 2026-07 test session) showed setInterval firing 30-80ms late
        ...
        // be late, as long as it runs before the buffer empties).
      ...
      _advanceSchedFrame() {
        ...
        this._schedFF++;
        if (this._schedFF >= fps) { this._schedFF = 0; this._schedSS++; }
        ...
        // Keep the "confirmed position" in step with what's been
        // scheduled — see constructor comment.
        this._hh = this._schedHH; this._mm = this._schedMM; this._ss = this._schedSS; this._ff = this._schedFF;
      }
