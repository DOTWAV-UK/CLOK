...
(function (global) {
  ...
  if (!global.ClokTC || !global.ClokState || !global.ClokEngine ||
    ...
    class MtcGenerator {
      ...
      start(hh, mm, ss, ff, fps, reason = 'start') {
        ...
        // DIAGNOSTIC (temporary): the start/stop/locate logging above only
        // shows state-machine transitions. If setInterval itself fires late
        // or in bursts under main-thread load, none of that logging would
        // catch it — the generator looks "running" the whole time even if
        // real quarter-frames are going out irregularly, which is exactly
        // the kind of gap that would make Pro Tools lose chase without any
        // start/stop/locate event ever firing. So: track actual elapsed
        // time between ticks and log whenever it's well outside expected.
        ...
        this._timer = setInterval(() => {
          ...
          // Expected gap is qfMs (~10ms @25fps). Flag anything more than
          // 3x that — a genuine stall, not just normal timer wobble.
          if (gap > qfMs * 3) {
            this._onEvent({ type: 'qf-gap', gapMs: Math.round(gap), expectedMs: Math.round(qfMs), t: now });
          }
          // Once a minute, report the worst gap seen so far even if nothing
          // crossed the flag threshold — confirms whether it's "clean" or
          // "borderline everywhere".
          if (tickCount % (60 * 4 * Math.round(this._fps)) === 0) {
            this._onEvent({ type: 'qf-summary', maxGapMs: Math.round(maxGap), t: now });
            maxGap = 0;
          }
          this._sendQF();
          if (this._qfIndex === 0) {
  ...
  function startReceive({ outputId } = {}) {
    ...
    generator = new MtcGenerator({
      ...
      onEvent: (ev) => {
        if (ev.type === 'qf-gap') {
          console.warn('[MTC-OUT] ⚠ QF GAP', ev.gapMs + 'ms (expected ~' + ev.expectedMs + 'ms)', JSON.stringify(ev));
        } else if (ev.type === 'qf-summary') {
          console.log('[MTC-OUT] — 1min summary — worst gap:', ev.maxGapMs + 'ms', JSON.stringify(ev));
        } else {
