...
const ClokState = {
  ...
  MSG: {
    ...
    // Browser → Bridge
    LTC_FRAME: 'ltc-frame',  // decoded LTC frame from browser
    PING:      'ping',
    PONG:      'pong',
  },
...
class MtcGenerator {
  ...
  update(hh, mm, ss, ff, fps) {
    ...
    // Allow incoming frame to be up to 8 frames behind the generator's current
    // position — the generator drifts slightly ahead between CLOK corrections,
    // so a small negative diff is normal and should not trigger a locate.
    ...
    if (diffFrames < -8 || diffFrames > 8) {
      // Genuine position jump — send Full Frame and reset QF counter
...
function openPorts() {
  ...
  if (MODE === 'send' && inIdx >= 0) {
    ...
    midiInput.ignoreTypes(false, false, false); // allow SysEx, timing (MTC QFs 0xF1), active sensing
  ...
  ws.on('message', raw => {
    try {
      ...
      if (msg.type === ClokState.MSG.PING) {
        ws.send(JSON.stringify({ type: ClokState.MSG.PONG, mode: MODE }));
      } else if (msg.type === ClokState.MSG.LTC_FRAME && MODE === 'receive') {
        onLtcFrame(msg);
      } else if (msg.type === 'tc_frame' && MODE === 'send') {
        // Generator relay: broadcast generator TC to all other clients (e.g. SuiteView)
        broadcast(msg);
      }
    } catch(e) {}
  });
...
// In send mode we do NOT use the engine's wall-clock interpolation.
// Node's event loop + IAC latency means performance.now() drifts significantly
// relative to MTC frame timing — the engine extrapolates the wrong position
// between frames, causing visible drift that grows ~60ms/frame.
// Instead: broadcast the raw decoded MTC position directly on each QF frame.
...
const parser = new MtcParser({
  ...
  onFrame(p) {
    ...
    // Log every ~24 frames so terminal isn't flooded
    if (p.ff === 0) {
      log(chalk.green(`MTC  ${p.tc}  ${p.fps}fps`));
    }

    // Broadcast raw decoded position — no wall-clock interpolation
    broadcast({
      type:    ClokState.MSG.TC_FRAME,
      hh:      p.hh, mm: p.mm, ss: p.ss, ff: p.ff,
      fps:     p.fps,
      df:      p.df,
      playing: true,
      state:   ClokState.STATE.LOCKED,
      source:  ClokState.SOURCE.MTC,
      tc:      ClokTC.format(p.hh, p.mm, p.ss, p.ff),
      locating: p.locating || false,
      driftMs: 0, corrMs: 0, seqNum: 0, wallMs: 0,
      v:       CLOK_VERSION,
    });
  ...
  onLocate(p) {
    ...
    broadcast({
      ...
      df:      p.df,
      playing: isPlaying,
      state:   ClokState.STATE.LOCATING,
      source:  ClokState.SOURCE.MTC,
      tc:      ClokTC.format(p.hh, p.mm, p.ss, p.ff),
      locating: true,
      driftMs: 0, corrMs: 0, seqNum: 0, wallMs: 0,
  ...
  onStop() {
    ...
    isPlaying = false;
    broadcast({ type: ClokState.MSG.TRANSPORT, state: ClokState.STATE.STOPPED, playing: false });
...
function startBroadcast() {
  ...
  // Keepalive only — real frame data comes from onFrame above
  broadcastTimer = setInterval(() => {
    if (!isPlaying || !lastMtcPos) return;
    // just re-send last known position so browser doesn't time out
    const p = lastMtcPos;
    broadcast({
      type:    ClokState.MSG.TC_FRAME,
      hh:      p.hh, mm: p.mm, ss: p.ss, ff: p.ff,
...
// ══════════════════════════════════════════════════════════════════════════════
//
// Ported directly from the working v2 implementation.
// Key design: qfOutput is a shared state object. CLOK updates it on every
// decoded frame. The QF interval reads from it — no separate advancing timer
// fighting CLOK's position corrections.

const FPS_CODE_RX = { 24:0, 25:1, 29.97:2, 30:3, 23.976:0 };
...
function sendFullFrameRx(hh, mm, ss, ff, fpsCode) {
  const hr = ((fpsCode & 0x03) << 5) | (hh & 0x1F);
  midiOutput.sendMessage([0xF0, 0x7F, 0x7F, 0x01, 0x01, hr, mm&0x3F, ss&0x3F, ff&0x1F, 0xF7]);
  log(chalk.magenta(`\u27BF  Full Frame SysEx \u2192 ${pad(hh)}:${pad(mm)}:${pad(ss)}:${pad(ff)} @ ${[24,25,29.97,30][fpsCode] ?? fpsCode} fps`));
}

function isSequentialRx(prev, next, fps) {
...
function onLtcFrame(msg) {
  ...
  const fpsCode  = FPS_CODE_RX[fps] !== undefined ? FPS_CODE_RX[fps] : FPS_CODE_RX[25];
  const fps_r    = Math.round(fps || 25);
  const qfMs     = (1000 / (fps || 25)) / 8;
  const newTC    = { hh, mm, ss, ff };

  if (!isSequentialRx(qfOutput.prevTC, newTC, fps_r)) {
    sendFullFrameRx(hh, mm, ss, ff, fpsCode);
    qfOutput.qfIndex = 0;
  }

  Object.assign(qfOutput, {
    hh, mm, ss, ff, fps: fps || 25, fpsCode,
...
function startQFStream(qfMs) {
  ...
  qfOutput.interval = setInterval(() => {
    ...
    midiOutput.sendMessage([0xF1, buildQFByte(qfIndex, hh, mm, ss, ff, fpsCode)]);
    qfOutput.qfIndex = (qfIndex + 1) % 8;
    if (qfOutput.qfIndex === 0) {
      const fps_r = Math.round(qfOutput.fps);
      if (++qfOutput.ff >= fps_r) {
        qfOutput.ff = 0;
        if (++qfOutput.ss >= 60) {
          qfOutput.ss = 0;
          if (++qfOutput.mm >= 60) {
...
function armStopTimeout() {
  ...
  // Pro Tools often doesn't send 0xFC — detect stop by QF silence
  // 300ms = ~7 frames at 25fps, enough to distinguish stop from jitter
  stopTimeout = setTimeout(() => {
    if (isPlaying) {
      log(chalk.yellow('Transport stop (QF timeout)'));
      isPlaying = false;
      broadcast({ type: ClokState.MSG.TRANSPORT, state: ClokState.STATE.STOPPED, playing: false });
      if (broadcastTimer) { clearInterval(broadcastTimer); broadcastTimer = null; }
    }
...
midiInput.on('message', (deltaTime, message) => {
  ...
  // Re-arm stop timeout on every incoming QF or SysEx
