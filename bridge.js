...
function openPorts() {
  ...
  if (MODE === 'send' && inIdx >= 0) {
    ...
    midiInput.ignoreTypes(false, true, true); // pass SysEx+MTC (0xF1/0xF0); filter MIDI clock + active sensing
...
const engine = new ClokEngine({ fps: 25, pll: { snapFrames: 8, histLen: 4, gain: 0.3 } });
...
const parser = new MtcParser({
  ...
  onFrame(p) {
    ...
    // Broadcast on every incoming frame — don't wait for the timer.
    // Chase mode LTC output must track MTC position directly.
...
function startBroadcast() {
  ...
  // Keepalive only — real frame broadcasts happen directly in onFrame above.
