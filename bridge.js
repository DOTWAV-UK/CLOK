  ...
  ws.on('message', raw => {
    try {
      ...
      if (msg.type === ClokState.MSG.PING) {
        ...
        onLtcFrame(msg);
      } else if (msg.type === ClokState.MSG.LTC_FRAME && MODE === 'send') {
        // Generator relay: CLOK browser app sends ltc-frame messages from its
        // internal generator. Relay them to SuiteView as canonical tc_frame broadcasts.
        // Build a full tc_frame payload so SuiteView's onmessage handler picks it up.
        const { hh = 0, mm = 0, ss = 0, ff = 0, fps = 25 } = msg;
        broadcast({
          ...
          source:  ClokState.SOURCE.LTC,
        });
      } else if (msg.type === 'tc_frame' && MODE === 'send') {
        // Pass-through: forward any tc_frame already in canonical form
        broadcast(msg);
      }
    } catch(e) {}
  });
