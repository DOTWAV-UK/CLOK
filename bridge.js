  ...
  ws.on('message', raw => {
    try {
      ...
      if (msg.type === ClokState.MSG.PING) {
        ...
        // Generator → bridge relay: CLOK generator sends its current TC here.
        // Rebroadcast to all OTHER connected clients (e.g. SuiteView) as tc_frame
        // using the canonical ClokState.payload() shape.
        const { hh = 0, mm = 0, ss = 0, ff = 0, fps = 25 } = msg;
        const payload = ClokState.payload({
          hh, mm, ss, ff, fps,
          state:   ClokState.STATE.LOCKED,
          source:  ClokState.SOURCE.INTERNAL,
          playing: true,
        });
        const out = JSON.stringify({ type: ClokState.MSG.TC_FRAME, ...payload });
